    ...
    try:
        ...
        if cauth_info['service']['version'] > '0.2.0':
            ...
            if (username
                ...
                methods = ','.join(cauth_info['service']['auth_methods'])
                raise ValueError(m % methods)
            header = {'Content-Type': 'application/json'}
            if use_ssl:
                url = "https://" + url
                resp = requests.post(url, auth_params, allow_redirects=False,
                                     verify=verify,
                                     headers=header)
            else:
                url = "http://" + url
