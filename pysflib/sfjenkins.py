...
class SFJenkins(jenkins.Jenkins):
    def __init__(self, url, username=None, password=None, cookie=None,
                 ...
                 insecure=False):
        # Assuming we enforce SSL everywhere as of SF 2.2.6
        ...
        if j_url[-1] != '/':
            j_url += '/'
        super(SFJenkins, self).__init__(j_url + 'jenkins/', username,
                                        password, timeout)
        if not cookie and (not username or not password):
            raise ValueError("Authentication needed")
        if not cookie:
            self.cookie = sfauth.get_cookie(c_url, username, password,
                                            use_ssl=True, verify=not insecure)
        else:
            # raise Warning("Cookie will take precedence over credentials")
            self.cookie = cookie
